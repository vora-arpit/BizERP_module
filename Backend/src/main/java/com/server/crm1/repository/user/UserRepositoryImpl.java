// package com.server.crm1.repository.user;

// import java.util.Arrays;
// import java.util.Collections;
// import java.util.List;

// import org.springframework.beans.factory.annotation.Autowired;
// import org.springframework.jdbc.core.namedparam.MapSqlParameterSource;
// import org.springframework.jdbc.core.namedparam.NamedParameterJdbcTemplate;

// import com.server.crm1.model.users.User;


// import com.nimbusds.oauth2.sdk.util.StringUtils;

// public class UserRepositoryImpl implements UserRepositoryCustom {

// 	@Autowired
// 	private NamedParameterJdbcTemplate jdbc;

// 	@Override
//     public List<User> search(String filter) {
//     String filterParam = StringUtils.isBlank(filter) ? "" : " and (x.name ilike :text or email ilike ':text') ";
//     String sql = "select \n" +
//             "   x.id ,\n" +
//             "   x.name ,\n" +
//             "   x.email ,\n" +
//             "   x.image_Url,\n" +
//             "   STRING_AGG (r.name,', ') roles \n" +
//             " from users x \n" +
//             " left outer join user_role ru on ru.user_id=x.id \n" +
//             " left outer join role r on ru.role_name=r.name \n" +
//             " where true \n" +
//             filterParam +
//             " group by x.id \n" +
//             " order by x.name \n";
//     MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
//     mapSqlParameterSource.addValue("text", "%" + filter + "%");
//     return jdbc.query(
//             sql,
//             mapSqlParameterSource,
//             (rs, rowNum) -> {
//                 int idx = 1;
//                 User user = new User();
//                 user.setId(rs.getInt(idx++));
//                 user.setName(rs.getString(idx++));
//                 user.setEmail(rs.getString(idx++));
//                 user.setImageUrl(rs.getString(idx++));
//                 String rolesSTR = rs.getString(idx++);
//                 if (rolesSTR != null) {
//                     user.setRoles(Arrays.asList(rolesSTR.split(", ")));
//                 } else {
//                     user.setRoles(Collections.emptyList()); // Set empty list if rolesSTR is null
//                 }
//                 return user;
//             });
// }

// }


package com.server.crm1.repository.user;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.jdbc.core.namedparam.MapSqlParameterSource;
import org.springframework.jdbc.core.namedparam.NamedParameterJdbcTemplate;

import com.server.crm1.model.users.User;
import com.nimbusds.oauth2.sdk.util.StringUtils;

public class UserRepositoryImpl implements UserRepositoryCustom {

    @Autowired
    private NamedParameterJdbcTemplate jdbc;

    @Override
    public List<User> search(String filter, Long organizationId) {
        String filterParam = StringUtils.isBlank(filter) ? "" : " and (x.name ilike :text or email ilike ':text') ";
        String sql = "select \n" +
                "   x.id ,\n" +
                "   x.name ,\n" +
                "   x.email ,\n" +
                "   x.image_Url,\n" +
                "   STRING_AGG (r.name,', ') roles \n" +
                " from users x \n" +
                " left outer join user_role ru on ru.user_id=x.id \n" +
                " left outer join role r on ru.role_name=r.name \n" +
                " where true \n" +
                filterParam +
                " and x.organization_id = :organizationId \n" + 
                " group by x.id \n" +
                " order by x.name \n";
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("text", "%" + filter + "%");
        mapSqlParameterSource.addValue("organizationId", organizationId);  
        return jdbc.query(
                sql,
                mapSqlParameterSource,
                (rs, rowNum) -> {
                    int idx = 1;
                    User user = new User();
                    user.setId(rs.getInt(idx++));
                    user.setName(rs.getString(idx++));
                    user.setEmail(rs.getString(idx++));
                    user.setImageUrl(rs.getString(idx++));
                    String rolesSTR = rs.getString(idx++);
                    if (rolesSTR != null) {
                        user.setRoles(Arrays.asList(rolesSTR.split(", ")));
                    } else {
                        user.setRoles(Collections.emptyList()); 
                    }
                    return user;
                });
    }


    @Override
    public List<User> search(String filter) {
    String filterParam = StringUtils.isBlank(filter) ? "" : " and (x.name ilike :text or email ilike ':text') ";
    String sql = "select \n" +
            "   x.id ,\n" +
            "   x.name ,\n" +
            "   x.email ,\n" +
            "   x.image_Url,\n" +
            "   x.organization_id,\n" + // Include organization_id in the select query
            "   STRING_AGG (r.name,', ') roles \n" +
            " from users x \n" +
            " left outer join user_role ru on ru.user_id=x.id \n" +
            " left outer join role r on ru.role_name=r.name \n" +
            " where true \n" +
            filterParam +
            " group by x.id \n" +
            " order by x.name \n";
    MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
    mapSqlParameterSource.addValue("text", "%" + filter + "%");
    return jdbc.query(
            sql,
            mapSqlParameterSource,
            (rs, rowNum) -> {
                int idx = 1;
                User user = new User();
                user.setId(rs.getInt(idx++));
                user.setName(rs.getString(idx++));
                user.setEmail(rs.getString(idx++));
                user.setImageUrl(rs.getString(idx++));
                user.setOrganizationId(rs.getLong(idx++)); // Set organizationId from the result set
                String rolesSTR = rs.getString(idx++);
                if (rolesSTR != null) {
                    user.setRoles(Arrays.asList(rolesSTR.split(", ")));
                } else {
                    user.setRoles(Collections.emptyList()); // Set empty list if rolesSTR is null
                }
                return user;
            });
    }



}